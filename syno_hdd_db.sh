        ...
        if [[ -f ${synoinfo}.bak ]]; then
            ...
            for v in "${!keyvalues[@]}"; do
                ...
                if [[ $currentval != "$defaultval" ]]; then
                    if /usr/syno/bin/synosetkeyvalue "$synoinfo" "${keyvalues[v]}" "$defaultval"; then
...
for i in "${!eunits[@]}"; do
    ...
    eunitdb1list+=($(find "$dbpath" -maxdepth 1 -regextype posix-extended -iregex ".*${eunits[i],,}(_v7)?.db"))
    #eunitdb2list+=($(find "$dbpath" -maxdepth 1 -name "${eunits[i],,}*.db.new"))
    eunitdb2list+=($(find "$dbpath" -maxdepth 1 -regextype posix-extended -iregex ".*${eunits[i],,}(_v7)?.db.new"))
done
...
if [[ $dsm -gt "6" ]]; then # DSM 6 as has no dmidecode
    if [[ $ram == "yes" ]] && [[ -f /usr/sbin/dmidecode ]]; then
        ...
        if [[ $ramtotal =~ ^[0-9]+$ ]]; then                                     # Check $ramtotal is numeric
            ...
            elif [[ $setting -gt "$ramtotal" ]] && [[ $setting -gt "$settingbak" ]]; then # GitHub issue #107
